...
(function() {
    ...
    // ============================================
    // PERMANENT STORAGE (load + refresh)
    // ============================================
    function clearPublicGrid() {
    ...
    function loadAllImages() {
        ...
        // If there were no images, counter/empty state is handled by addImageToGallery
        // but since grid was cleared, ensure empty message exists.
        if (images.length === 0) updatePublicCounter();
    }

    // Refresh UI when admin changes gallery
    window.addEventListener('fh-admin-gallery-changed', () => {
        // small debounce to allow multiple uploads
        if (window.__fhGalleryRefreshTimer) clearTimeout(window.__fhGalleryRefreshTimer);
        window.__fhGalleryRefreshTimer = setTimeout(() => loadAllImages(), 50);
    });
    ...
    function init() {
        ...
        // Optional Admin entry UI (admin-panel.js is responsible for rendering once authed)
        // This keeps the admin feature accessible from the main site.
        try {
            const adminBtn = document.createElement('button');
            adminBtn.id = 'fh-admin-entry-btn';
            adminBtn.type = 'button';
            adminBtn.innerHTML = '<i class="fas fa-user-shield"></i> Admin';
            adminBtn.style.position = 'fixed';
            adminBtn.style.right = '20px';
            adminBtn.style.top = '110px';
            adminBtn.style.zIndex = '1001';
            adminBtn.style.background = '#c87e3a';
            adminBtn.style.color = 'white';
            adminBtn.style.border = 'none';
            adminBtn.style.borderRadius = '999px';
            adminBtn.style.padding = '0.8rem 1rem';
            adminBtn.style.fontWeight = '900';
            adminBtn.style.cursor = 'pointer';
            adminBtn.style.boxShadow = '0 10px 18px rgba(200,126,58,0.25)';

            const adminRootId = 'fh-admin-root';
            const ensureAdminRoot = () => {
                let r = document.getElementById(adminRootId);
                if (!r) {
            ...
            adminBtn.addEventListener('click', () => {
                // admin-panel.js renders itself when authenticated; we just provide a click target.
                // Create the root container so admin-panel can mount into it.
                ensureAdminRoot();
                // If already authed, force a refresh so admin UI appears.
                // This is harmless for users; admin UI visibility is gated by localStorage.
                try {
